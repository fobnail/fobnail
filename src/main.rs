...
fn main() -> ! {
    pal::init();
    let trussed_platform = pal::trussed::init();
    let mut trussed_platform_service = trussed::service::Service::new(trussed_platform);
    let trussed_platform_client = trussed_platform_service
        .try_as_new_client("fobnail_client")
        .unwrap();
    ...
    let mut fobnail_client = FobnailClient::new(coap_client, trussed_platform_client);
