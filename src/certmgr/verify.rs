...
impl CertMgr {
    ...
    fn tcg_compliance_check(cert: &X509Certificate) -> bool {
        ...
        //
        // Currently we are not using it alternative name so we just ignore it.
        //
        // Subject alternative name contains various intereseting
        ...
        if Self::ca_constaint_check(cert).0 {
    ...
    fn ca_constaint_check(cert: &X509Certificate) -> (bool, Option<u8>) {
        ...
        T: trussed::client::Sha256,
    {
        ...
        let parent_key = if let Some(parent) = parent {
            let (ca, path_len_constraint) = Self::ca_constaint_check(parent);
            if !ca {
            ...
            if !Self::ca_constaint_check(child).0 {
